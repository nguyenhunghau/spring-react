...
export const Control = ({ id, idLayout, text, moveCard, moveControl, findLayout, findControl, wasDrop, setWasDrop }) => {
  const layout = findLayout(idLayout);
  // const currentIdLayout = layout.id;
  const originalIndex = findControl(layout, id).index;
  const [{ isDragging }, drag] = useDrag({
    item: { type: ItemTypes.CONTROL, id, idLayout, originalIndex },
    collect: (monitor) => ({
